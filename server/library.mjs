...
export function createLibrary(ctx){
 ...
 async function handler(req,res,path,method,auth){
  ...
  if(m){let item=owned(m[1],auth);
   if(['preview','file'].includes(m[2])&&['GET','HEAD'].includes(method)){const payload=JSON.parse(item.payload),r=payload.item;if(item.kind!=='resource')fail(404,'Vista no disponible.');if(m[2]==='preview'){if(!['html','lab'].includes(r.kind))fail(404,'Vista no disponible.');const content=r.content||(r.file_key?readFileSync(join(uploadsDir,r.file_key),'utf8'):'');res.setHeader('Content-Security-Policy',"sandbox allow-scripts; default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src https: data:; font-src data:; connect-src 'none'; form-action 'none'; base-uri 'none'; frame-ancestors 'self'");res.setHeader('Content-Type','text/html; charset=utf-8');res.end(method==='HEAD'?undefined:content);return true;}if(!r.file_key)fail(404,'Archivo no encontrado.');fileResponse(req,res,join(uploadsDir,r.file_key),r.file_mime,r.file_name,!['application/pdf'].includes(r.file_mime)&&!r.file_mime.startsWith('image/'),true);return true;}
   if(!m[2]&&method==='GET'){const payload=JSON.parse(item.payload);return send({id:item.id,title:item.title,kind:item.kind,shared:!!item.shared,preview:item.kind==='module'?{title:payload.item.title,materials:payload.resources.map(r=>({title:r.title,kind:r.kind})),activities:payload.activities.map(r=>JSON.parse(r.item.config).title)}:item.kind==='activity'?JSON.parse(payload.item.config):{title:payload.item.title,kind:payload.item.kind,content:payload.item.content,url:payload.item.url,fileName:payload.item.file_name}});}
