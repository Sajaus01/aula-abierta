import {setupAcademicPolicy,revisionDecision,validatePeriods,gradeCalculation} from './academic-policy.mjs';
...
export function createAcademics(ctx) {
 const {db,fail,json,readJson,readSession,requireAdmin,requireStudent,requireCourse,isEnrolled,validateFile,fileResponse,uploadsDir,audit,env,accessModel}=ctx;
 const all=(sql,...p)=>db.prepare(sql).all(...p),one=(sql,...p)=>db.prepare(sql).get(...p),run=(sql,...p)=>db.prepare(sql).run(...p);
 if(accessModel)setupAcademicPolicy(db);
 ...
 function view(a,auth) {const available=admin(auth)||!a.opensAt||a.opensAt<=now();return {...a,instructionUrl:available?a.instructionUrl||'':'',attachments:available?files.publicFiles(files.instructionFiles(a.id),`/api/academics/activities/${a.id}/attachments`):[],materialsAvailable:available,questions:!admin(auth)&&a.opensAt&&a.opensAt>now()?[]:a.questions.map(q=>admin(auth)?q:{id:q.id,type:q.type,prompt:q.prompt,options:q.options,points:q.points,imageUrl:q.imageUrl}),submissionCount:admin(auth)?one("SELECT COUNT(*) n FROM submissions WHERE activity_id=? AND state!='draft' AND superseded=0",a.id).n:undefined,activeResponseCount:admin(auth)?one('SELECT COUNT(*) n FROM submissions WHERE activity_id=? AND superseded=0',a.id).n:undefined};}
 function subView(s,auth) {const p=JSON.parse(s.payload),attachments=submissionFiles(p),first=attachments[0];const visible=admin(auth)||Boolean(s.published);const prior=accessModel?one('SELECT config,files FROM activity_versions WHERE activity_id=? AND revision=?',s.activity_id,s.revision):null;const versionConfig=prior?JSON.parse(prior.config):getActivity(s.activity_id);return {questionFeedback:visible?versionConfig.questions.map(q=>({id:q.id,feedback:q.feedback||''})):[],instructionFiles:prior?files.publicFiles(JSON.parse(prior.files),`/api/academics/submissions/${s.id}/instructions`):[],id:s.id,activityId:s.activity_id,revision:s.revision,superseded:Boolean(s.superseded),previousActivity:admin(auth)&&prior?JSON.parse(prior.config):p.previousActivity||null,studentId:s.student_id,requestId:s.request_key,attempt:s.attempt,state:s.state==='graded'&&!visible?'submitted':s.state,text:p.text||'',url:p.url||'',answers:p.answers||{},files:files.publicFiles(attachments,`/api/academics/submissions/${s.id}/files`),fileName:first?.name||null,fileSize:first?.size||0,fileUrl:first?`/api/academics/submissions/${s.id}/file`:null,submittedAt:s.submitted_at,late:Boolean(s.late),points:visible?s.points:null,feedback:visible?s.feedback:'',published:Boolean(s.published),version:s.version,updatedAt:s.updated_at};}
 ...
 function gradeRow(activities,userId,cfg,forAdmin) {
  if(accessModel)return gradeCalculation(activities,userId,cfg,forAdmin,{rows,closed,override:(activityId,studentId)=>Boolean(one("SELECT exempt FROM grade_overrides WHERE activity_id=? AND student_id=?",activityId,studentId)?.exempt)});
 ...
 function config(body,current=null) {
  ...
  if(c.kind==='quiz'){
   ...
   c.questions=b.questions.map(q=>{const id=text(q.id,'Identificador',80,true);if(ids.has(id))fail(400,'Identificador de pregunta repetido.');ids.add(id);if(!['single','multiple','text'].includes(q.type))fail(400,'Tipo de pregunta no válido.');
    ...
    if(q.type!=='text'){if(!Array.isArray(q.options)||q.options.length<2||q.options.length>10)fail(400,'Cada pregunta de selección requiere entre 2 y 10 opciones.');r.options=q.options.map(v=>text(v,'Opción',1000,true));if(!Array.isArray(q.correct)||!q.correct.length||q.correct.some(n=>!Number.isInteger(n)||n<0||n>=r.options.length)||new Set(q.correct).size!==q.correct.length||(q.type==='single'&&q.correct.length!==1))fail(400,'Selecciona las respuestas correctas.');r.correct=[...q.correct].sort((a,b)=>a-b);}if(accessModel){r.imageUrl=text(q.imageUrl??'','Imagen',2048);if(r.imageUrl&&!/^https:\/\/[^\s]+$/.test(r.imageUrl))fail(400,'La imagen requiere una URL HTTPS.');r.feedback=text(q.feedback??'','Retroalimentación',3000);}return r;});
  ...
  if(accessModel){c.periodId=text(b.periodId??'','Corte',80);c.timeLimitMinutes=num(b.timeLimitMinutes??0,'Tiempo límite',0,180);c.repeatPolicy=current?.repeatPolicy||'keep';}
  return c;
 }
 function checkWeights(courseId,c,id=null) {if(accessModel&&c.periodId&&!(settings(courseId).periods||[]).some(p=>p.id===c.periodId))fail(400,'Selecciona un corte existente.');const total=list(courseId).filter(a=>a.id!==id&&a.status==='published'&&(!accessModel||(a.periodId||'')===(c.periodId||''))).reduce((n,a)=>n+a.weight,0)+(c.status==='published'?c.weight:0);if(total>100.00001)fail(400,'Los porcentajes de actividades publicadas no pueden superar el 100 %.');}
 ...
 async function handler(req,res,path,method,auth){
  ...
  if(path==='/api/academics/courses'&&method==='GET'){
   if(admin(auth))return send(all('SELECT id,title FROM courses ORDER BY title').filter(c=>!accessModel||accessModel.p.can(auth.user.id,c.id,'view')).map(c=>({...c,pending:one("SELECT COUNT(*) n FROM submissions s JOIN activities a ON a.id=s.activity_id WHERE a.course_id=? AND s.superseded=0 AND (s.state='submitted' OR (s.state='graded' AND s.published=0))",c.id).n})));
  ...
  if(match){const [,id,section]=match;const c=access(id,auth);
   ...
   if(section==='settings'&&method==='PUT'){const b=await readJson(req);auth=readSession(req);requireAdmin(auth);access(id,auth);const cfg={...settings(id),...b};const out={scaleMax:num(cfg.scaleMax,'Escala',1,100),passMark:num(cfg.passMark,'Nota aprobatoria',0,cfg.scaleMax),missingAsZero:bool(cfg.missingAsZero,'Pendientes como cero'),finalPublished:bool(cfg.finalPublished,'Publicar notas finales')};
    if(accessModel){out.periods=validatePeriods(cfg.periods||[],fail);out.rounding='2 decimales';const activities=list(id).filter(a=>a.status==='published'),students=all("SELECT student_id FROM enrollments WHERE course_id=?",id);const impact=students.map(s=>({studentId:s.student_id,before:gradeRow(activities,s.student_id,settings(id),true).provisional,after:gradeRow(activities,s.student_id,out,true).provisional}));if(b.preview)return send({settings:out,impact});if(impact.some(r=>r.before!==r.after)&&!b.confirmImpact)fail(409,'Revisa y confirma el efecto del cambio sobre las notas.');}
    if(out.finalPublished){const activities=list(id).filter(a=>a.status==='published');const students=all("SELECT student_id FROM enrollments WHERE course_id=? AND status='active'",id);if((!out.periods?.length&&Math.abs(activities.reduce((n,a)=>n+a.weight,0)-100)>0.00001)||students.some(s=>!gradeRow(activities,s.student_id,out,true).ready))fail(400,'Para publicar la nota final, los porcentajes deben sumar 100 % y cada actividad debe estar calificada y publicada. Las entregas faltantes solo cuentan como cero si activas esa opción y el plazo terminó.');}saveSettings(id,out);audit(auth.user,'grades.settings',id);return send(out);}
  }
  ...
  if(match){const [,id,section]=match;let a=checkActivity(id,auth);
   if(!section&&method==='GET'){let deadline=null;if(accessModel&&!admin(auth)&&!auth.preview&&a.timeLimitMinutes>0&&(!a.opensAt||a.opensAt<=now())&&!closed(a)){run('INSERT OR IGNORE INTO attempt_starts VALUES(?,?,?,?)',id,auth.user.id,a.revision,now());const start=one('SELECT started_at FROM attempt_starts WHERE activity_id=? AND student_id=? AND revision=?',id,auth.user.id,a.revision);deadline=new Date(Date.parse(start.started_at)+a.timeLimitMinutes*60000).toISOString();}const data={deadline,activity:view(a,auth),course:one('SELECT id,title FROM courses WHERE id=?',a.courseId)};if(!admin(auth)){data.mine=rows(id,auth.user.id).map(s=>subView(s,auth));data.previous=all('SELECT * FROM submissions WHERE activity_id=? AND student_id=? AND superseded=1 ORDER BY revision DESC,attempt DESC',id,auth.user.id).map(s=>subView(s,auth));}return send(data);}
   if(!section&&method==='DELETE'){
    ...
    const attached=[...(accessModel?all('SELECT files FROM activity_versions WHERE activity_id=?',id).flatMap(r=>JSON.parse(r.files)):[]),...files.instructionFiles(id),...all('SELECT payload FROM submissions WHERE activity_id=?',id).flatMap(s=>submissionFiles(JSON.parse(s.payload)))];
    db.exec('BEGIN IMMEDIATE');try{if(accessModel){run('DELETE FROM grade_audit WHERE submission_id IN (SELECT id FROM submissions WHERE activity_id=?)',id);run('DELETE FROM learning_events WHERE object_id=?',id);}run('DELETE FROM activities WHERE id=?',id);invalidate(a.courseId);audit(auth.user,'activity.delete',id);db.exec('COMMIT');}catch(error){if(db.isTransaction)db.exec('ROLLBACK');throw error;}
   ...
   if(!section&&method==='PATCH'){
    ...
    const decision=accessModel?revisionDecision(a,c,b,count,plan.added.length>0||plan.removed.length>0,fail):null;const reset=decision?decision.reset:Boolean(count&&(changed||b.requestResubmission));if(decision)c.repeatPolicy=decision.policy;let reopened=false;
    ...
    if(accessModel&&b.preview){const oldActivities=list(a.courseId).filter(x=>x.status==='published'),nextActivities=list(a.courseId).map(x=>x.id===id?{...x,...c}:x).filter(x=>x.status==='published');const impact=all('SELECT u.id,u.name FROM users u JOIN enrollments e ON e.student_id=u.id WHERE e.course_id=?',a.courseId).map(u=>({name:u.name,before:gradeRow(oldActivities,u.id,settings(a.courseId),true).provisional,after:gradeRow(nextActivities,u.id,settings(a.courseId),true).provisional}));return send({impact,affectedResponses:count,policy:decision.policy,changed:decision.changed,oldWeight:a.weight,newWeight:c.weight,oldPeriod:a.periodId||'',newPeriod:c.periodId||''});}
    saveActivityFiles(plan,id,()=>{
     if(accessModel){if(changed&&b.changeKind==='evaluated')run("UPDATE submissions SET superseded=1,version=version+1 WHERE activity_id=? AND state='draft' AND superseded=0",id);run('INSERT OR IGNORE INTO activity_versions VALUES(?,?,?,?,?)',id,a.revision,JSON.stringify(a),JSON.stringify(files.instructionFiles(id)),now());run("UPDATE submissions SET payload=json_set(payload,'$.previousActivity',json(?)) WHERE activity_id=? AND json_extract(payload,'$.previousActivity') IS NULL",JSON.stringify(snapshot),id);}
     if(reset){if(accessModel)run('UPDATE submissions SET superseded=1,version=version+1 WHERE activity_id=? AND superseded=0',id);else run("UPDATE submissions SET superseded=1,payload=json_set(payload,'$.previousActivity',json(?)),version=version+1 WHERE activity_id=? AND superseded=0",JSON.stringify(snapshot),id);}
   ...
   if(section==='submit'&&method==='POST'){
    ...
    if(accessModel&&a.timeLimitMinutes>0){const start=one('SELECT started_at FROM attempt_starts WHERE activity_id=? AND student_id=? AND revision=?',id,auth.user.id,a.revision);if(!start)fail(409,'Abre la actividad para iniciar el tiempo del intento.');if(Date.now()>Date.parse(start.started_at)+a.timeLimitMinutes*60000)fail(403,'El tiempo de este intento terminó. Solicita una nueva oportunidad al docente.');}
    if(previous&&b.version!==previous.version)fail(409,'Tu borrador cambió en otra pestaña. Recarga antes de guardar.');
    const history=rows(id,auth.user.id);if(history.filter(s=>s.state!=='draft'&&(!accessModel||a.repeatPolicy!=='optional'||s.revision===a.revision)).length>=a.maxAttempts)fail(403,'Alcanzaste el máximo de entregas o intentos.');if(!previous&&history.some(s=>s.state==='draft'))fail(409,'Ya tienes un borrador. Recarga y continúa ese borrador.');
    ...
    files.write(plan);try{db.exec('BEGIN IMMEDIATE');
     if(previous)run('UPDATE submissions SET payload=?,state=?,submitted_at=?,late=?,points=?,revision=?,version=version+1,updated_at=? WHERE id=?',JSON.stringify(payload),final?(points===null?'submitted':'graded'):'draft',final?now():null,Number(final&&a.dueAt&&now()>a.dueAt)||0,points,a.revision,now(),sid);
     ...
     if(final){if(accessModel)run('DELETE FROM attempt_starts WHERE activity_id=? AND student_id=? AND revision=?',id,auth.user.id,a.revision);invalidate(a.courseId);audit(auth.user,'submission.submit',sid);}else if(accessModel&&!previous)audit(auth.user,'submission.start',sid);db.exec('COMMIT');
  ...
  match=/^\/api\/academics\/submissions\/([^/]+)\/(grade|file|files|instructions)(?:\/([^/]+))?$/.exec(path);
  if(match){const [,id,section]=match;let s=one('SELECT * FROM submissions WHERE id=?',id);if(!s)fail(404,'Entrega no encontrada.');const a=checkActivity(s.activity_id,auth);if(!admin(auth)&&s.student_id!==auth.user.id)fail(403,'Esta entrega pertenece a otro estudiante.');
   if(section==='instructions'&&accessModel&&['GET','HEAD'].includes(method)){const v=one('SELECT files FROM activity_versions WHERE activity_id=? AND revision=?',s.activity_id,s.revision),file=v&&JSON.parse(v.files).find(f=>f.id===match[3]);if(!file)fail(404,'Enunciado histórico no encontrado.');return serve(req,res,file);}
