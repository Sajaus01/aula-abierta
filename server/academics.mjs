...
export function createAcademics(ctx) {
 ...
 const getActivity=id=>{const r=one('SELECT * FROM activities WHERE id=?',id);if(!r)fail(404,'Actividad no encontrada.');return {...parse(r),id:r.id,courseId:r.course_id,moduleId:r.module_id,version:r.version,createdAt:r.created_at};};
 const list=id=>all('SELECT * FROM activities WHERE course_id=? ORDER BY created_at,id',id).map(r=>getActivity(r.id));
 const checkActivity=(id,auth)=>{const a=getActivity(id);access(a.courseId,auth);if(!admin(auth)&&a.status!=='published')fail(404,'Actividad no disponible.');return a;};
 function view(a,auth) {return {...a,questions:!admin(auth)&&a.opensAt&&a.opensAt>now()?[]:a.questions.map(q=>admin(auth)?q:{id:q.id,type:q.type,prompt:q.prompt,options:q.options,points:q.points}),submissionCount:admin(auth)?one("SELECT COUNT(*) n FROM submissions WHERE activity_id=? AND state!='draft'",a.id).n:undefined};}
 function subView(s,auth) {const p=JSON.parse(s.payload);const visible=admin(auth)||Boolean(s.published);return {id:s.id,activityId:s.activity_id,studentId:s.student_id,requestId:s.request_key,attempt:s.attempt,state:s.state==='graded'&&!visible?'submitted':s.state,text:p.text||'',url:p.url||'',answers:p.answers||{},fileName:p.file?.name||null,fileSize:p.file?.size||0,fileUrl:p.file?`/api/academics/submissions/${s.id}/file`:null,submittedAt:s.submitted_at,late:Boolean(s.late),points:visible?s.points:null,feedback:visible?s.feedback:'',published:Boolean(s.published),version:s.version,updatedAt:s.updated_at};}
 function rows(id,userId) {return all('SELECT * FROM submissions WHERE activity_id=? AND student_id=? ORDER BY attempt DESC,updated_at DESC',id,userId);}
 ...
 function removeFile(file){if(file)try{unlinkSync(join(uploadsDir,file.key));}catch(e){if(e.code!=='ENOENT')console.error('No se pudo retirar un adjunto.');}}
 ...
 async function handler(req,res,path,method,auth){
  ...
  if(path==='/api/academics/storage'&&method==='GET'){requireAdmin(auth);return send({usedBytes:one("SELECT COALESCE(SUM(json_extract(payload,'$.file.size')),0) n FROM submissions").n,maxBytes:maxStorage,fileMaxBytes:10*1024*1024});}
  if(path==='/api/academics/courses'&&method==='GET'){
   if(admin(auth))return send(all('SELECT id,title FROM courses ORDER BY title').map(c=>({...c,pending:one("SELECT COUNT(*) n FROM submissions s JOIN activities a ON a.id=s.activity_id WHERE a.course_id=? AND (s.state='submitted' OR (s.state='graded' AND s.published=0))",c.id).n})));
   ...
   return send(courses.map(c=>({id:c.id,title:c.title,activities:list(c.id).filter(a=>a.status==='published').map(a=>({id:a.id,title:a.title,dueAt:a.dueAt,opensAt:a.opensAt,closesAt:a.closesAt,kind:a.kind,state:rows(a.id,auth.user.id).find(s=>s.state!=='draft')?'Entregada':'Pendiente'}))})));
  }
  ...
  if(match){const [,id,section]=match;const c=access(id,auth);
   if(!section&&method==='GET'){const acts=list(id).filter(a=>admin(auth)||a.status==='published');const cfg=settings(id);return send({course:{id:c.id,title:c.title},settings:cfg,activities:acts.map(a=>({...view(a,auth),mine:!admin(auth)?rows(a.id,auth.user.id).map(s=>subView(s,auth)):undefined})),grade:!admin(auth)?gradeRow(acts,auth.user.id,cfg,false):null});}
   requireAdmin(auth);
   if(section==='release'&&method==='POST'){await readJson(req);auth=readSession(req);requireAdmin(auth);access(id,auth);db.exec('BEGIN IMMEDIATE');try{run("UPDATE submissions SET published=1,version=version+1 WHERE points IS NOT NULL AND activity_id IN (SELECT id FROM activities WHERE course_id=? AND json_extract(config,'$.status')='published')",id);invalidate(id);audit(auth.user,'grades.release',id);db.exec('COMMIT');}catch(e){db.exec('ROLLBACK');throw e;}return send({success:true});}
   if(section==='activities'&&method==='POST'){const b=await readJson(req);auth=readSession(req);requireAdmin(auth);access(id,auth);const a=config(b);checkWeights(id,a);let moduleId=b.moduleId||null;if(moduleId&&!one('SELECT id FROM modules WHERE id=? AND course_id=?',moduleId,id))fail(400,'Capítulo no válido.');const aid=randomUUID();run('INSERT INTO activities(id,course_id,module_id,config,created_at) VALUES (?,?,?,?,?)',aid,id,moduleId,JSON.stringify(a),now());invalidate(id);audit(auth.user,'activity.create',aid);return send(view(getActivity(aid),auth),201);}
  ...
  if(match){const [,id,section]=match;let a=checkActivity(id,auth);
   if(!section&&method==='GET'){const data={activity:view(a,auth),course:one('SELECT id,title FROM courses WHERE id=?',a.courseId)};if(!admin(auth))data.mine=rows(id,auth.user.id).map(s=>subView(s,auth));return send(data);}
   if(!section&&method==='PATCH'){requireAdmin(auth);const b=await readJson(req);auth=readSession(req);requireAdmin(auth);a=checkActivity(id,auth);if(b.version!==a.version)fail(409,'La actividad cambió. Recarga antes de guardar.');const c=config(b,a);if(one('SELECT id FROM submissions WHERE activity_id=?',id)&&(a.kind!==c.kind||a.maxPoints!==c.maxPoints||JSON.stringify(a.questions)!==JSON.stringify(c.questions)))fail(409,'Con entregas guardadas, conserva el tipo, las preguntas y los puntos. Crea otra actividad para cambiarlos.');checkWeights(a.courseId,c,id);const moduleId=b.moduleId===undefined?a.moduleId:b.moduleId||null;if(moduleId&&!one('SELECT id FROM modules WHERE id=? AND course_id=?',moduleId,a.courseId))fail(400,'Capítulo no válido.');run('UPDATE activities SET config=?,module_id=?,version=version+1 WHERE id=?',JSON.stringify(c),moduleId,id);invalidate(a.courseId);audit(auth.user,'activity.edit',id);return send(view(getActivity(id),auth));}
   if(section==='submissions'&&method==='GET'){requireAdmin(auth);return send({activity:view(a,auth),submissions:all("SELECT s.*,u.name,u.document FROM submissions s JOIN users u ON u.id=s.student_id WHERE activity_id=? AND s.state!='draft' ORDER BY s.submitted_at DESC,s.attempt DESC",id).map(s=>({...subView(s,auth),name:s.name,document:s.document}))});}
   if(section==='submit'&&method==='POST'){
    requireStudent(auth);const b=await readJson(req,15*1024*1024);auth=readSession(req);requireStudent(auth);a=checkActivity(id,auth);const key=text(b.requestId,'Identificador de entrega',80,true);if(!/^[\w-]{8,80}$/.test(key))fail(400,'Identificador de entrega no válido.');if(!['draft','submit'].includes(b.action))fail(400,'Acción no válida.');
    const previous=one('SELECT * FROM submissions WHERE activity_id=? AND student_id=? AND request_key=?',id,auth.user.id,key);if(previous&&previous.state!=='draft')return send(subView(previous,auth));checkWindow(a);
    ...
    const oldFile=previous?JSON.parse(previous.payload).file:null;payload.file=b.removeFile?null:oldFile;let newFile;
    if(a.kind==='quiz'&&(b.file||payload.url||payload.text))fail(400,'Responde el cuestionario en sus preguntas.');
    if(b.file){if(!/\.(pdf|png|jpe?g|webp|xlsx|csv|docx|pptx|txt)$/i.test(b.file.name||''))fail(400,'Adjunta PDF, imagen, XLSX, CSV, DOCX, PPTX o TXT.');newFile=validateFile(b.file);if(newFile.size>10*1024*1024)fail(413,'Cada entrega admite un archivo de hasta 10 MB.');const used=one("SELECT COALESCE(SUM(json_extract(payload,'$.file.size')),0) n FROM submissions").n;if(used+newFile.size-(oldFile?.size||0)>maxStorage)fail(413,'El espacio para entregas está lleno. Comunícate con tu docente o entrega un enlace.');const disk=statfsSync(uploadsDir);if(Number(disk.bavail)*Number(disk.bsize)<newFile.size+50*1024*1024)fail(413,'No hay espacio disponible. Entrega un enlace o consulta a tu docente.');payload.file={key:newFile.key,name:newFile.name,mime:newFile.mime,size:newFile.size};}
    if(a.kind==='task'&&b.action==='submit'&&!payload.text&&!payload.url&&!payload.file)fail(400,'Añade texto, un enlace o un archivo antes de entregar.');
    ...
    try{if(newFile)writeFileSync(join(uploadsDir,newFile.key),newFile.buffer,{flag:'wx',mode:0o600});db.exec('BEGIN IMMEDIATE');
     ...
     else run('INSERT INTO submissions(id,activity_id,student_id,request_key,attempt,state,payload,submitted_at,late,points,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)',sid,id,auth.user.id,key,attempt,final?(points===null?'submitted':'graded'):'draft',JSON.stringify(payload),final?now():null,Number(final&&a.dueAt&&now()>a.dueAt)||0,points,now());
     if(final){invalidate(a.courseId);audit(auth.user,'submission.submit',sid);}db.exec('COMMIT');
    }catch(e){if(db.isTransaction)db.exec('ROLLBACK');if(newFile)removeFile(newFile);throw e;}
    if(oldFile&&oldFile.key!==payload.file?.key)removeFile(oldFile);return send(subView(one('SELECT * FROM submissions WHERE id=?',sid),auth),201);
   }
  }
  match=/^\/api\/academics\/submissions\/([^/]+)\/(grade|file)$/.exec(path);
