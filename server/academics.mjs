...
export function createAcademics(ctx) {
 ...
 async function handler(req,res,path,method,auth){
  ...
  if(match){const [,id,section]=match;let a=checkActivity(id,auth);
   ...
   if(!section&&method==='PATCH'){requireAdmin(auth);const b=await readJson(req,29*1024*1024);auth=readSession(req);requireAdmin(auth);a=checkActivity(id,auth);if(b.version!==a.version)fail(409,'La actividad cambió. Recarga antes de guardar.');const c=config(b,a);if(one('SELECT id FROM submissions WHERE activity_id=?',id)&&(a.kind!==c.kind||a.maxPoints!==c.maxPoints||JSON.stringify(a.questions)!==JSON.stringify(c.questions)))fail(409,'Con entregas guardadas, conserva el tipo, las preguntas y los puntos. Crea otra actividad para cambiarlos.');checkWeights(a.courseId,c,id);const moduleId=b.moduleId===undefined?a.moduleId:b.moduleId||null;if(moduleId&&!one('SELECT id FROM modules WHERE id=? AND course_id=?',moduleId,a.courseId))fail(400,'Capítulo no válido.');const plan=files.prepare(files.instructionFiles(id),b.attachments,b.retainAttachmentIds,{teacher:true});saveActivityFiles(plan,id,()=>{run('UPDATE activities SET config=?,module_id=?,version=version+1 WHERE id=?',JSON.stringify(c),moduleId,id);invalidate(a.courseId);audit(auth.user,'activity.edit',id);});return send(view(getActivity(id),auth));}
