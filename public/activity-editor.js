import {supportEditor,readSupportEditor,refreshSupportEditor,supportPreview,supportRules} from './procedure-support.js';
...
export function evaluationStructure(a){return JSON.stringify({kind:a.kind,maxPoints:Number(a.maxPoints),procedureSupport:supportRules(a.procedureSupport),questions:(a.kind==='quiz'?a.questions:[]).map(q=>({id:q.id,type:q.type,options:q.type==='text'?[]:q.options,correct:q.type==='text'?[]:[...q.correct].sort((a,b)=>a-b),points:Number(q.points)}))});}
...
export function bindActivityEditor(form,a,c,evaluation){
 ...
 function refresh(){
  ...
  if(a.id&&field('changeKind')&&a.activeResponseCount){
   const structure={kind:field('kind').value,procedureSupport:readSupportEditor(form),maxPoints:quiz?Number(points.toFixed(2)):Number(field('maxPoints').value),questions:questions.map(q=>({id:q.dataset.question,type:q.querySelector('[name=qType]').value,points:Number(q.querySelector('[name=qPoints]').value),options:q.querySelector('[name=qOptions]').value.split('\n').map(s=>s.trim()),correct:q.querySelector('[name=qCorrect]').value.split(',').filter(Boolean).map(v=>Number(v)-1)}))};
 ...
 function validate(){
  ...
  if(field('procedureEnabled').checked&&!form.querySelector('[name=procedureFormats]:checked'))fail(form.querySelector('[name=procedureFormats]'),'Selecciona al menos un formato para los soportes.');
