...
let pdf,loadingTask,current=1,renderTask,generation=0;
...
try{const url=new URL(new URLSearchParams(location.search).get('file')||'',location.origin);if(url.origin!==location.origin||!/^\/api\/academics\/(?:activities\/[^/]+\/attachments\/[^/]+|submissions\/[^/]+\/files\/[^/]+)$/.test(url.pathname))throw Error('Ruta no válida');
 ...
 loadingTask=getDocument({url:url.pathname,isEvalSupported:false,enableXfa:false,cMapUrl:'/pdfjs/cmaps/',cMapPacked:true,standardFontDataUrl:'/pdfjs/standard_fonts/',wasmUrl:'/pdfjs/wasm/'});pdf=await loadingTask.promise;
...
let resize;window.addEventListener('resize',()=>{clearTimeout(resize);resize=setTimeout(()=>{if(zoom.value==='fit')render();},150);});window.addEventListener('pagehide',()=>{clearTimeout(resize);generation++;renderTask?.cancel();loadingTask?.destroy().catch(()=>{});});
